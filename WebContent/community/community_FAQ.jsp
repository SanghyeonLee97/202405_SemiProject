...
<body>
	<%
		CommunityDAO ndao = new CommunityDAO();
			ArrayList<CommunityFAQDTO> noticeArr = new ArrayList();
		...
		communitySearch=request.getParameter("search");	//검색어 변수를 search파라미터로
			...
			noticeArr=ndao.getFAQList(communitySelect,communitySearch);	//검색실행
		...
		<section>
			...
			<article>
				<table>
					...
					%>
					<tr>
						<td><%=noticeArr.get(i).getFaqNo() %></td>
						<td><%=noticeArr.get(i).getFaqIQCNo() %></td>
						<td id="title" 
						onclick="location.href='/projectdengdeng/community/community_FAQ_read.jsp?FAQNo=<%=noticeArr.get(i).getFaqNo() %>'">
						<%=noticeArr.get(i).getFaqTitle() %></td>
					</tr>
					<%} %>
				</table>
			</article>
